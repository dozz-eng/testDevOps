...
- участвует в разборе нештатных ситуаций работы приложения на основе данных логгирования и мониторинга. Чем оптимальнее настроены данные компоненты, тем меньше времени уходит на разбор.
...
В более сложных системах есть этапы настройки инфраструктуры для теста, развертывание компонентов приложения, выполнение дополнительных действий (перезапуск сервисов, вызов дополнительных сервисов), Логирование и отправка оповещений о состоянии поставки и т.д.

3. **Мониторинг и логирование:**
    
Мониторинг является одной из самых важных работ в сопровождении приложения. До того, как приложение "упадет", могут быть различные к этому предпослки (мало места на жестком диске, например). Система может дать оповещение обслуживающему персоналу, чтобы тот мог предпринять необходимые меры. 
...
- Мониторинг производится в реальном времени, и оповещения присылаются сразу по наступлении важного события. 
...
Ввиду отсутствия собственного ядра системы и гипервизора ресурсы хостовой машины используются более рационально, увеличивая быстродействие системы в целом. 
...
Изначально протокол IP предусматривал около 4 млрд адресов в формате xxx.xxx.xxx.xxx, но с ростом количества устройств в мире стало невозможно выдать каждому устройству индивидуальный адрес. 
...
- Шлюз записывает в таблицу IP-адрес отправителя, и заменяет его в пакете на свой внешний IP-адрес.
...
На заре развития интернета, когда количество компьютеров можно было пересчитать по пальцам, обращение к ресурсу в интернете производилось напрямую по IP-адресу. Соответствие имени IP-адресу настраивалось в каждом компьютере в файле hosts.

С ростом количества устройств, поддерживать актуальное состояние файлов hosts стало невозможно. На смену файлам появились DNS-сервера, которые взяли на себя эту функцию. На каждом компьютере осталось только настроить IP-адрес DNS-сервера. При вводе в браузере имени ресурса, комьютер сначала проверяет, есть ли у нео в кэше информация об адресе и, если нет, он отправляет запрос на DNS-сервер. 
...
Разработчик делает новую ветку из master, называет ее, например, feature01 и начинает изменять код. Изменения не отражаются в основной ветке. Добаавив функционал, проверив его, запушив соммит на сервер, разработчик может инициировать слияние своей ветки с основной. Эта операция называется pull-request (или  merge-request в gitlab). После выполнения pull-request'а код в основной ветке сливается с кодом в ветке feature01. Функционал добавлен.

Для создания новой ветки и переключения на нее следует набрать команду:
...
- выдача пользователям сервиса минималього набора прав, необходимых для работы.
- не работать под пользователем root. Заблокировать удаленное подключение пользователя root на сервере.
- физический доступ в серверные комнаты должен быть жестко ограничен. 
...
Приватный должен находиться в надежном месте и его никому нельзя передавать.
...
Согласно манифесту, система организует заданное количество контейнеров, производит сборку, деплой, запуск, перезапуск, балансировку нагрузки между контейнерами,  масштабирование системы в зависимости от нагрузки. Система может отслеживать состояние контейнера, и в случае некорректного поведения перезапускать его. 

Самый распространенный оркестратор контейнеров - Kubernetes.
Менее распространенные: Docker Swarm, Openshift, Rancher

9. **Docker-compose**

Данный файл запускает два сервиса web и db. Имена контейнеров сервиса будут выбраны случайным образом.

Сервисы: 

- web - контейнер с сервером nginx и пробросом порта 8080 компьютера на порт 80 контейнера. 
...
Чтобы добавить том для хранения данных необходимо дописать в секцию "db" следующую строку:

    volumes:
    - /home/<user>/rebrain/db/data:/var/lib/postgresql/data

Это один из нескольких вариантов подключения volume. 
...
В папке web находится Dockerfile c командами для сборки образа python3.8, зависимости в образе устанавливаются в соответствии с файлом requirements.txt. 

В корне проекта находится файл docker-compose.yaml, посредством которого производится сборка проекта. В файле описаны три сервиса: web, frontend и db

- web - для сборки образа используется Dockerfile в папке web. Порт 8000  контейнера пробрасывается на порт 8001 хостовой машины.

- frontend - для сборки образа используется абстрактная директория ./frontend. Контейнер запускается командой "run 0.0.0.0". Порт 3000  контейнера пробрасывается на порт 3000 хостовой машины.

- db - контейнер собирается на базе стандартного образа postgres:16-alpine из dockerhub. Для хранения базы данных подключена локалная директория ./db_django.

Для корректного запуска сервиса в проекте задействованы healthcheck'и. Первым должен запуститься контейнер с базой данных, затем контейнер с django, за ним - frontend.